...
const strategies = ref([
  ...
  { icon: '🚌', name: '跨校区通勤', desc: '跨校区通勤上课，仙林↔鼓楼/苏州', note: '单程约40-60分钟，适合每周1-2次课程', enabled: true },
...
function toggle(s: (typeof strategies.value)[number]) {
  s.enabled = !s.enabled
}
...
onMounted(async () => {
  ...
  strategies.value.forEach(s => (s.enabled = saved.includes(s.name)))
})
...
<template>
  <div class="page">
    ...
    <section class="card strategy-card">
      <template v-for="(s, index) in strategies" :key="s.name">
        <button class="strategy" :class="{ enabled: s.enabled }" :aria-pressed="s.enabled" @click="toggle(s)">
          <span class="strategy-icon">{{ s.icon }}</span>
          <span class="strategy-body">
            <span class="strategy-name">{{ s.name }}</span>
            <span class="strategy-desc">{{ s.desc }}</span>
            <span class="strategy-note">{{ s.note }}</span>
          </span>
          <span class="toggle" :class="{ on: s.enabled }" aria-hidden="true">
            <span class="knob"></span>
          </span>
        </button>
        <div v-if="index < strategies.length - 1" class="strategy-divider" aria-hidden="true" />
      </template>
    </section>
...
.strategy-card { padding: var(--space-2) var(--space-4); }
.strategy { display: flex; align-items: flex-start; gap: var(--space-3); width: 100%; padding: var(--space-3) 0; border: none; background: transparent; color: inherit; cursor: pointer; font: inherit; text-align: left; }
.strategy-icon { font-size: var(--text-2xl); flex-shrink: 0; }
.strategy-body { min-width: 0; flex: 1; display: flex; flex-direction: column; }
...
.strategy-divider { height: 1px; background: var(--bg-subtle); }
.toggle { width: 44px; height: 26px; position: relative; flex-shrink: 0; border-radius: var(--radius-full); background: var(--bg-green-soft); }
.knob { position: absolute; top: 3px; left: 3px; width: 20px; height: 20px; border-radius: 50%; background: #fff; box-shadow: 0 1px 3px rgba(0,0,0,.12); transition: left .2s; }
